  ...
  if (plusBtn && plusMenu) {
    ...
    plusMenu.addEventListener("click", (e) => {
      ...
      if (action === "social") {
        ...
        const ORDER = ["haru", "greeter"];
        const curIdx = ORDER.indexOf(
          typeof currentCharacterKey !== "undefined" ? currentCharacterKey : "haru"
        );
        ...
        // char-* 클래스 정리
        if (ghostElLocal) {
          ghostElLocal.classList.remove("char-greeter");
          if (nextKey !== "haru") ghostElLocal.classList.add("char-" + nextKey);
        }
        ...
        const greetings = {
          haru:    "안녕! 나 하루야. 오늘도 같이 얘기 많이 하자!",
          greeter: "어서 오세요, 접수원 하루입니다! 무엇을 도와드릴까요?"
        };
        if (typeof setEmotion === "function") {
